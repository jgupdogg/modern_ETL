"""
Silver Wallet PnL DAG
Calculates profit and loss metrics for whale wallets using PySpark
"""
import os
import json
import time
...
def get_silver_pnl_schema() -> StructType:
    """Get PyArrow schema for silver PnL data"""
    return StructType([
...
def calculate_token_pnl_udf(transactions):
    ...
    Args:
        transactions: List of transaction records for a wallet-token pair
        
    Returns:
        Struct with PnL metrics
    """
    if not transactions:
...
def read_unfetched_transactions(spark: SparkSession) -> DataFrame:
    ...
    try:
        ...
        # Use path pattern to exclude JSON status files
...
def update_bronze_processing_status(spark: SparkSession, processed_df: DataFrame, batch_id: str):
    ...
    try:
        ...
        # Add date partition for proper organization
...
def calculate_wallet_pnl_batch(**context):
    """
    Main PySpark job for calculating wallet PnL metrics
    """
    logger = logging.getLogger(__name__)
...
dag = DAG(
    ...
    description='Calculate PnL metrics for whale wallets using PySpark',
    schedule_interval='0 */12 * * *',  # Every 12 hours
    catchup=False,
    tags=['silver', 'pnl', 'pyspark', 'analytics'],
)

# Define tasks
pnl_calculation_task = PythonOperator(
    task_id='calculate_wallet_pnl',
    python_callable=calculate_wallet_pnl_batch,
    provide_context=True,
    dag=dag,
)

# This DAG calculates comprehensive PnL metrics for whale wallets using FIFO methodology