"""
Bronze Wallet Transactions DAG
Fetches transaction history for whale wallets from BirdEye API
"""
import os
import json
import time
...
def fetch_wallet_transactions(**context):
    """
    Task to fetch transaction history for whale wallets and store in MinIO bronze layer
    """
    logger = logging.getLogger(__name__)
    ...
    for _, whale_row in whales_df.iterrows():
        ...
        try:
            ...
            response = birdeye_client.get_wallet_trade_history(
                wallet_address=wallet_address,
...
dag = DAG(
    ...
    description='Fetch transaction history for whale wallets from BirdEye API',
    schedule_interval='0 */6 * * *',  # Every 6 hours
    catchup=False,
    tags=['bronze', 'transactions', 'birdeye'],
)

# Define tasks
fetch_transactions_task = PythonOperator(
    task_id='fetch_wallet_transactions',
    python_callable=fetch_wallet_transactions,
    provide_context=True,
    dag=dag,
)

# This DAG fetches transaction history for whale wallets marked as unfetched in the bronze layer